...
def importnotes(client_id, client_item_id):
    notes = gethighrisenotes(client_id)
    #note_id: update_id
    updates = {}
    for note in notes:
        if 'parent_id' in note:
            comment_body = highriseauthors[note['author']] + '\n' + note['body'] + '\n' + note['created']
            create_comment = 'mutation { create_update (item_id: ' + str(client_item_id) + ', parent_id: ' + updates[note['parent_id']] + ' body: "' + comment_body + '") { id } }'
            data = {'query': create_comment}
            requests.post(url=apiUrl, json=data, headers=mondayheaders)
            continue
        update_body = highriseauthors[note['author']] + '\n' + note['body'] + '\n' + note['created']
        create_update = 'mutation { create_update (item_id: ' + str(client_item_id) + ', body: "' + update_body + '") { id } }'
